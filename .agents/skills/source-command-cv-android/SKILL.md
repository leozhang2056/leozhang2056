---
name: "source-command-cv-android"
description: "Generate CV for Android roles"
---

# source-command-cv-android

Use this skill when the user asks to run the migrated source command `cv-android`.

## Command Template

Generate a CV for Android roles.

Usage: `/cv-android --jd-file <path> --company "<name>" --title "<title>"`

```bash
python generate.py cv --role android --jd-file <path> --company "<company>" --title "<title>" --with-quality-report --with-jd-annotated
```

##同步生成 Cover Letter (MANDATORY)

```bash
python generate.py cl --role android --jd-file <path> --company "<company>" --title "<title>"
```
