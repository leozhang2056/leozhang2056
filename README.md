...
## Resume-Matcher 思路整合（已落地）

已将 `Resume-Matcher` 里最有价值、且适配本仓库的能力整合为本地优先工作流：

- 主简历（结构化 KB）+ JD 定制：由 `kb/` + `projects/*/facts.yaml` 驱动，避免“手工改一份又一份简历”。
- JD 关键词匹配与反幻觉过滤：仅保留能被 KB 证据支撑的关键词，再用于排序与生成。
- JD Annotated PDF：默认输出命中高亮、匹配分、命中/未命中列表，便于快速审阅。
- Cover Letter 生成：基于同一份 KB 事实，保持与简历叙事一致。
- Application Email 生成（新增）：支持根据角色/JD 生成投递邮件文本。
