...
|----|------|------|
| Part 1 | **Core Behavioral Answers**: 10 highest-frequency primary answers — start here | [→](#section-oral-only) |
| Part 2 | **Project Story Bank**: Story warehouse grouped by project + B1/B2 classification index | [→](#section-part-b) |
| Part 3 | **Question Script Bank**: Q1–Q45 full scripts + deep-dive details | [→](#section-5-scripts) |
| Part 4 | **Quick Review & Appendix**: Tools, quick reference, material drafts & inbox | [→](#section-toolbox) / [→](#section-appendix) |

> Fine-grained anchors (e.g. `oral-01`, `b1-03`, `q-12`, `d3-04`) are located **above each section heading**; when needed, search for `id="q-12"` or open the corresponding Part and browse by headings like `### 12)`.
...
This document is for **behavioral interview** preparation: **Part 1** contains 10 highest-frequency primary answers, **Part 2** is the story bank, **Part 3** has Q1–Q45 full scripts/deep dives, **Part 4** is quick reference/tools.

### Rules (add your own below)

- **Think about why they're asking** before answering the point (don't just pile on experience).
- **Facts**: Numbers and experiences only come from `kb/*.yaml`, `projects/*/facts.yaml`; new facts mentioned in conversation should be persisted to the project.
- **Spoken delivery**: Short sentences, easy to pronounce; **common words** preferred; **Section A** for main practice, **Section C** as dictionary; adapt on the spot, don't sound scripted.
- **Length**: **①, ⑩** prepare **30s + 1min** two versions; **②–⑨** and **Section C** each question **one version** oral script is enough, expand or shorten on the spot.
...
### A. On-the-Spot Opening

> **Layout**: **①, ⑩** = **30s + 1min**; **②–⑨** = **one version per question**. Below, **STAR prompts** are in English keywords, the rest is oral script.
...
> I'm a senior full-stack engineer with over ten years of experience architecting and delivering robust systems across backend, mobile, and IoT platforms.
>
> Two years ago I came to New Zealand to pursue my Master's at AUT, specializing in applied AI — my thesis focused on diffusion models and production-ready machine learning workflows.
>
...
- **Identity**: Senior full-stack engineer, 10+ years
...
**Key idea:** Android battery optimization restrictions, whitelist + multi-channel fallback.

#### Primary Script (Recommended)
> One of the toughest technical challenges I faced was ensuring an Android messaging app stayed alive in the background.
>
> Unlike shopping or podcast apps, our app had to deliver messages instantly, even when the system's battery optimization killed background services.
>
> The first and most reliable step was guiding users to whitelist the app from battery optimization. We also built a layered approach: FCM for primary messaging, JPush as a supplementary wake-up channel, and client-side tuning for heartbeat, session recovery, and state recognition. The combination delivered much more reliable background message delivery.

#### Key Points (for quick recall)
- **Problem**: Android messaging app had to deliver messages instantly, but the OS kept killing background services.
- **Why different from other apps**: Shopping or podcast apps don't need instant delivery — messaging does.
- **Solution**:
  - Guide users to whitelist the app from battery optimization.
  - Use alternative push services as backup channels.
- **Result**: Reliable message delivery even with aggressive battery optimization.
- **Lesson**: Background reliability for real-time apps requires multiple layers, not just default push.

**(Extended) Enterprise IM — Android OEM / version fragmentation**  
The messy part wasn't only logic — it was **where** the app ran. **Enterprise IM** hits a consumer‑style long tail: many OS levels, and in China **Xiaomi**, **Huawei**, **OPPO** ROMs all behave differently.

We kept routine QA on **mainstream** devices and versions (roughly **Android 7/8** as the lab floor). When bugs only showed up on user devices, we **bought the target phone** or worked with users to reproduce in their real settings.

The usage surface is too wide to pre‑catch every small defect without huge time, people, and device costs. So we balanced **release speed**, **test depth**, and **hardware budget**. We shipped on strong mainstream gates, then closed repro on the exact device path when signals were strong.

<a id="oral-enterprise-files"></a>
### (Supplement) Enterprise IM — Self-hosted File Platform (FastDFS)

**Key idea:** When single-server file storage can't handle the load, explain FastDFS, self-hosted layering, and access control.

#### Primary Script (Recommended)
> When we faced scaling storage while maintaining data privacy for the enterprise messaging system, I designed a self-hosted file layer using FastDFS.
>
> The goal was to support growing uploads — from chat attachments to design drawings — without overwhelming a single server or relying entirely on public cloud storage.
>
> Operationally, we focused on consistent access control across features and balanced performance with security and compliance.

#### Key Points (for quick recall)
- **Problem**: Single file server couldn't handle growing attachments/documents.
- **Constraint**: Privacy and data control; public cloud storage not default.
- **Solution**: Self-hosted **FastDFS** with scale-out storage, redundancy, shared document backbone.
- **Integration**: User, Messaging, and workflow subsystems shared one document layer.
- **Ops**: Sharding, backup/restore, consistent access control across features.
- **Lesson**: File systems require capacity + reliability + security together.

*(Layering note: after a Windows avatar upload incident, we tightened controls and moved some user-facing blobs to cloud object storage. Business documents still stayed on on-prem/self-managed FastDFS.)*

<a id="oral-03"></a>
### ③ Conflict with a teammate

**Key idea:** Focus on how 1:1 and feedback approach changed, not who was right or wrong.

#### Primary Script (Recommended)
> I encountered a conflict with a teammate during code reviews, where they reacted defensively. I realized the issue wasn't their ability, but past bad experiences impacting their perspective.
>
> To resolve this, I transitioned to one-on-one private feedback and focused on the code's impact, along with short pairing sessions.
>
> This led to faster reviews and a more stable release process, reinforcing that trust and feedback style are critical for success.

#### Key Points (for quick recall)
- **Situation**: Enterprise Messaging project; code reviews became tense; teammate reacted defensively.
- **Action**:
  - Moved feedback to **1:1** private conversation.
  - Realized it wasn't ability — past bad experiences made comments feel personal.
  - Changed approach: private feedback, focus on code impact, short pairing sessions.
- **Result**: Tone improved, reviews faster, release process more stable.
- **Lesson**: Trust is the real bottleneck; feedback style matters.

#### Case B (Optional: Tech Stack Disagreement)
- **Situation**: Disagreement on technical direction; teammate preferred older tools (Delphi) for stability.
- **Action**: Discussed trade-offs openly (delivery risk, maintainability, support coverage).
- **Result**: Chose practical middle ground; legacy parts stayed, newer components introduced gradually.
- **Lesson**: Focus on operational impact/risk, not just "better" technology.

**(Expansion closer to your experience)**: **Delphi vs team mainstream stack**, **entry weighbridge/scale linkage** (overload alert, barrier/alarm), **professionally execute after decision maker rules + document to reduce risk** → see **[Q42 Option B](#q-42)** (don't fabricate client/project names before facts are stored).

**Case C (Optional: Cross-department Communication Conflict)**
**Key idea:** 讲清边界和折中，不要硬答“能”。有时候提的功能天花乱坠，没有先例，没有人这么做，可能并不适合用户
...
When developing the 9-grid feature

<a id="oral-04"></a>
### ④ Failure

**Key idea:** Pick a real mistake, explain the fix and prevention.

**Case A — Chinese herbal labeling (backup · estimation / delay / ambiguous requirements)**  
**Key idea:** **stakes relatively small**, not suitable to override the "biggest failure" main narrative; use for **missed deadline / estimation / ambiguity** follow-ups.  
...
**Case B — App release (test config shipped to production build · same source as main answer)**  
**Key idea:** Same narrative as **[Q4 Option B](#q-04)** — **accountability** + **weak process (manpower/memory)** + **hardening** + **don't skip gates even for small changes**.

One mistake I made earlier in my career happened during an application release.

I shipped an upgrade where the build was still pointing to **test** configs instead of **production**. Users saw incorrect data — they thought they'd lost their connections and contacts — and we had to prepare and ship a corrected version immediately. The pressure was real: when users call you because they think they've lost their data, you feel the weight of that mistake.

At that time we were a small team without a mature release process. Too much validation depended on manual checks and individual memory — and I was the only mobile developer, so everything rested on me.

I took responsibility, and afterward we transformed the release process. We separated dev/test/prod configs clearly, added multi‑person verification, and gradually introduced automation.

Since then I've been much more disciplined about releases, even for small updates. Reliable delivery needs systems and checks, not memory — and that experience taught me the hard way.

**Case C — Enterprise IM (user upload path · optional · default main answer only for security roles)**  
**Key idea:** Strong facts but **topic easily derails**; **don't use as default main answer** for non-**security / infra / IR** roles; if told, **downplay drama**, emphasize **boundaries and process learning** (same source as [Q4 Option C](#q-04)).
...
**Key idea:** First describe the source of pressure, then explain how you kept the rhythm.
...
**Case A — Smart Factory (release window)**  
...
**Case B — Enterprise IM (solo Android + post-launch firefighting · same source as [Q9](#q-09) main answer)**  
When I'm under pressure, I focus on reducing uncertainty early rather than reacting emotionally.

During the early release of an Enterprise Messaging app, I was the only Android developer. We launched fast, then a wave of bugs came in.

I didn't try to solve everything at once. Each day I prioritized by **user impact**, stabilized the highest‑risk problems first, and shipped smaller fixes continuously.

Product and support kept expectations clear with users while engineering focused on recovery.

That taught me that a calm, structured rhythm beats emotional reactions when things get hot.

**Case C — Air-gapped site (currency printing plant / backup · follow-up)**  
...
**Case D — Smart Factory update module (third-party upgrade failure)**  
**Key idea:** New device upgrade failure + third-party module bug + quick replacement + forming regression habits.
...
**Key idea:** Prioritize by impact / risk / dependency, then communicate early.

#### Key Points (for quick recall)
- **Situation**: Multiple urgent tasks (production issues vs milestones vs internal work).
- **Action**:
  - **Sort**: Impact first, then milestones, then rest.
  - **Communicate**: Honest about dates; clear order beats pretty plans.
  - **Execute**: Fix what hurts operators first; prototypes for unclear requirements.
  - **Adapt**: Redistribute work, reduce scope, adjust sequencing.
- **Result**: Predictable delivery; no late surprises; team trust maintained.
- **Lesson**: Prioritization is about visibility and early action, not just working harder.

Each day I looked at three things: the closest deadline, the biggest blocker, and which tasks could safely move without breaking trust.

I focused on impact and delivery risk, not just whoever messaged me last.

If it was too much for one person, I escalated early — pulled in help, paired on tough issues, or split ownership.

That kept multiple projects moving without constant chaos. Prioritization is as much about expectations as it is about tasks.

<a id="oral-07"></a>
### ⑦ Process improvement

**Key idea:** From manual to automated, explain why it's more reliable.

#### Key Points (for quick recall)
- **Situation**: Manual deployments (copying builds, syncing files); worked for small projects, but risky as **Smart Factory** expanded.
- **Action**:
  - Introduced **Jenkins + GitLab** for standardized CI/CD.
  - Piloted on **Smart Factory**, then rolled out to other projects.
  - Added lightweight checklists and docs to reduce tribal knowledge.
- **Result**: Releases became predictable, consistent, and easier to recover.
- **Lesson**: Improvement isn't just speed — it's consistency and reducing reliance on memory.
...
**Key idea:** First acknowledge the feedback was fair, then explain how you changed.

#### Key Points (for quick recall)
- **Situation**: Manager pointed out I kept too much context in my head; onboarding/maintenance got harder as team grew.
- **Action**:
  - Accepted feedback; started writing **lightweight decision notes** for technical choices.
  - Added **onboarding notes** for confusing modules.
  - Kept docs practical, not excessive.
- **Result**: Smoother onboarding; fewer repeated questions; better maintainability.
- **Lesson**: Engineering communication matters; write down context that affects future work.
...
- **Situation**: Different stakeholders wanted different things; conflicting directions; repeated changes.
- **Action**:
  - Used **quick prototypes** to get concrete feedback.
  - Aligned with **one customer-side decision maker** to prioritize requests.
- **Result**: Reduced scope churn; stable delivery target.

<a id="oral-09"></a>
### ⑨ Adapted to change

**Key idea:** Explain how constraints changed and how you adapted the approach.

#### Key Points (for quick recall)
- **Case A: ChatClothes (Thesis)**
  - **Situation**: Original design depended on cloud processing; evolved to need **offline capability** and **speed** for lightweight hardware (Raspberry Pi).
  - **Action**:
    - Measured real performance; original approach wouldn't meet constraints.
    - Moved AI pipeline **locally** (lightweight LLM, LoRA optimization).
    - Reduced non-essential scope; focused on practical responsiveness.
  - **Result**: System ran offline with better performance; finished with First Class Honours.
  - **Lesson**: Adapt to change by letting go of original design; measurements > attachment to early ideas.

- **Case B: Enterprise IM (Platform Evolution)**
  - **Situation**: Standalone messaging product evolved into a larger internal platform (shared identity, file services, permissions).
  - **Action**:
    - Moved core messaging from self-maintained C++ to **Easemob** for stability.
    - Focused effort on higher-level features: permissions, workflows, user systems, platform integration.
  - **Result**: Architecture adapted to scale and operational pressure; platform grew sustainably.

We also evolved file storage from a **single server** into a **distributed FastDFS‑based system** as attachments and documents grew.

At the same time, the **team process evolved** — more automation, better collaboration tools, clearer approval flows, and more structured coordination as the org grew.

I’ve learned that **good systems are rarely static**. As scale, cost, and operational reality change, the architecture and process have to evolve too — we stopped maintaining undifferentiated infrastructure and focused on the business logic and real product features instead.

**Oral exam boundary (aligned with [Q23](#q-23))**: For **significant change** questions, prioritize **Q23 main answer** to capture **"from standalone IM to shared infrastructure"** as one main thread; expand **vendor/component details** only on follow-up or see `projects/enterprise-messaging/facts.yaml`, avoid **conference-talk** style information overload.

<a id="oral-10"></a>
### ⑩ Why should we hire you?

**Key idea:** First explain what you can deliver, then why it fits the role.

#### Key Points (for quick recall)
- **Experience**: 13+ years across Android, Java backend, full-stack, and applied AI.
- **Delivery**: Comfortable with messy real-world problems; focus on reliability, rollout, and operational reality.
- **Recent**: AUT Master's (First Class Honours) — applied AI, lightweight offline deployment.
- **Fit**: Based in Auckland, full-time work rights; ready to contribute strong execution and practical problem-solving.
- **Value**: Combine engineering depth with AI skills; deliver systems that work in production, not just demos.
...
My thesis project, **ChatClothes**, focused on **running AI efficiently on lightweight hardware with offline support**. That reflects how I work: **measure constraints early**, **adapt the design when needed**, and focus on **practical usability** instead of unnecessary complexity.

I'm comfortable working across a system — **client**, **backend**, **deployment**, and **ops** — which helps when projects get messy or requirements change.

I'm based in **Auckland** with **full‑time work rights**, and I'm looking for a team where I can contribute **strong execution**, **reliability**, and **practical problem‑solving**.

#### One-Liner Version (for closing)
...
### (Supplement) Creative idea — Smart Factory (scale serial → WebSocket → auto-fill weight on web page)

**Key idea:** Explain the scale serial-to-WebSocket workflow optimization. **Same source as [Q7 Option B](#q-07)** compressed skeleton; if the question is **above and beyond**, still use **Q7 Option A (ChatClothes documentation)** as main answer.
...
**Key idea:** Reuse stories, avoid rewriting.

#### Key Points (for quick recall)
- **Purpose**: Reusable story bank; map stories to multiple questions.
- **Stories**:
  - **Smart Factory**: IoT, hardware integration, multi-site, efficiency gain.
  - **Enterprise Messaging**: Real-time, Android, backend, scalability.
  - **ChatClothes**: AI, offline, lightweight, First Class Honours.
- **Usage**: Pick the right story for the question; adapt details to fit the context.
...
### B1. High-Frequency Question Categories (English questions + Chinese prompts → now English prompts)

> **Quick lookup**: **To speak aloud** → [Oral section](#section-oral-only); **Keywords to memorize** → [B2 STAR](#section-2-star-brief); **Written dictionary (remember to adapt for speech)** → [Section C](#section-5-scripts).  
> **B1 navigation**: **B1-01** Motivation page [oral-b1-01](#oral-b1-01); **B1-02~B1-09** see links within each subsection **→ oral scripts**; **only the following are supplementary scripts** (no equivalent Q in Section C or need standalone paragraphs) → [oral-b1-05-supplement](#oral-b1-05-supplement) · [oral-b1-06-influence](#oral-b1-06-influence).

<a id="b1-01"></a>
### 1) Self-Awareness & Motivation
- Tell me about yourself. (**See [A ①](#oral-01)**: **30s + 1min** two versions; facts aligned with `kb/profile.yaml`)
- What are your strengths and weaknesses? (**See [B1-01 Oral Page · Strengths & Weaknesses](#oral-b1-01-sw)**; quick answer version also at [Q1](#q-01) · [Q2](#q-02))
- Why are you looking for a new opportunity? (**See [B1-01 Oral Page · Changing Jobs](#oral-b1-01-why-move)** = **Q32**)
- Why do you want to join this company? (**See [B1-01 Oral Page · Company Fit](#oral-b1-01-why-company)** = **Q29** template; **[Business + Role JD Alignment Memo](#oral-b1-01-why-company-research)**)

<a id="oral-b1-01"></a>
#### B1-01 Oral Page (Motivation · aligned with `kb/profile.yaml`)

<a id="oral-b1-01-sw"></a>
**What are your strengths and weaknesses?**

**Prompt**: 1 strength + **evidence**; 1 weakness + **actionable improvement** (avoid fake weaknesses like "I'm too perfect"). Use **manager feedback → lightweight decision notes / onboarding** for a real weakness story (see [A ⑧ Case A](#oral-08)).

#### Key Points (for quick recall)
- **Strength 1**: Ship across layers (Android, Java backend, integration) — Enterprise Messaging at scale, Smart Factory on shop floor.
- **Strength 2**: Measure before arguing — ChatClothes profiling, moved work local, ran offline on small hardware.
- **Weakness**: Kept too much context in head; onboarding/maintenance got harder.
- **Action**: Lightweight decision notes for technical choices; short onboarding notes for confusing modules.
- **Lesson**: Write down context that affects maintainability and future work; keep docs practical.
...
**Prompt** (aligned with your narrative): **Main reason is family** — relocated from **China** to **New Zealand**, wanting to **restart life in a new direction**; therefore **left previous company** (geographic and life planning, **not performance or personal conflict**). **After arriving in NZ**: Completed **AUT MCIS** **First Class** in **2026-02** (**ChatClothes**); now seeking **stable full-time** engineering role in **Auckland**. **Regarding previous role**: Still **value the work and skills accumulated**, next role hopes to **continue same capabilities** — Android / Java backend / enterprise & industrial delivery / applied AI; **don't disparage former employer**. **PSWV** full-time work rights (consistent with `kb/profile.yaml`).

#### Key Points (for quick recall)
- **Reason**: Family relocation from China to NZ; life direction change.
- **Recent**: AUT Master's (First Class Honours) completed Feb 2026; ChatClothes thesis.
- **Goal**: Stable full-time engineering role in Auckland.
- **Continuity**: Same direction — Android, Java backend, enterprise/industrial delivery, applied AI.
- **Rights**: PSWV (full-time work rights).
...
**Prompt (English key points · aligned with your narrative)**: ① **Management philosophy / work environment** — preference for trust, professional autonomy, and engineering culture; ② **Want to build "real products"** — users or frontline can directly feel the value (**Smart Factory** efficiency gains & reduced burden, **smart-power** real-time monitoring & data visibility); ③ **Applying to Air NZ** — at airline scale, **passenger-facing digital experience & reliable operations** can amplify technical value to more people (don't write unverified internal system names, avoid speculation). **Before interview**, make sure to do **[Business + Role JD Alignment](#oral-b1-01-why-company-research)**, anchoring motivation to **specific JD original wording**.

**Script — Air New Zealand (~ 30s）**
...
**Script — Air New Zealand (~ 60s，在 30s 上略展开）**
...
**Script — Alternative company name「Hotter」（若你指其他品牌，请把公司全名改成正式英文）**
...
**Script (generic template · other companies still use brackets)**
...
**Minimal one-liner (can memorize this)**
...
**Before Applying: Company Business + Role Requirements (Structured Memo)**

**Purpose**: Upgrade "why us" from **generic culture/real product** to **I read what business you're in + what this role requires + which of my experiences match one-to-one**. The most stable approach in oral exams is to **restate the must-haves from their JD**, then connect with your project evidence (avoid fabricating internal architecture).

#### A) Company Business (Air New Zealand · Public Information Layer)

| Dimension | What you should be able to articulate (don't memorize numbers) | Source |
| --- | --- | --- |
| **Customers & Product Form** | Primarily passenger transport, international & domestic network; traveler touchpoints include **App / Web**, itinerary & self-service; **day-of-travel** experience is sensitive to latency, clarity, stability | Website, press releases, annual reports/IR summaries |
| **Digital's Role in the Business** | Digital channels are **part of the product** (not just IT); operations side emphasizes **reliability, compliance, partner integration** | Careers site **[Digital Career Path](https://careers.airnewzealand.co.nz/belong-here/career-options/digital)** |
| **Engineering "Pain Point Types"** (summary) | High-concurrency read paths, offline/poor network, identity & payment peripherals, **disruption/rebooking** workflows, observability & release discipline | Combine JD + your common sense about travel products; **don't assert** undisclosed internal system names |

#### B) Role Requirements (JD → Oral Mapping Table)

1. **Deconstruct JD**: Separate `Must-have` / `Nice-to-have`; if leadership required, add `people / roadmap / stakeholders`.
2. **Make three columns** (print one page for pre-interview review): `JD original wording` | `Behavior/output they want` | `Your evidence (project + verb + result, facts from KB)`.
3. **Common mobile/full-stack JD skill clusters** (**generalized** from public market job descriptions, **use the one you applied to as authoritative**): Production-grade **Android (Kotlin / Jetpack / Compose)** or iOS; **REST / modern client architecture**; some roles list **BFF, AWS, CI/CD**, backend collaboration. When speaking, use **your posting mentions …** to reference specific wording.
4. **Project placeholder alignment** (check and rewrite per JD): Scaled mobile & integration → **enterprise-messaging**; real-time data & alerts/observability → **smart-power**; frontline business clarification & phased delivery → **Smart Factory**; on-device AI / thesis-level evidence → **ChatClothes**.

#### C) English Backup ("What do you know about our business?")
...
#### D) Alternative Employers (Hotter / Other)

Same process: **Website products/about us + that role's JD** → use one sentence to clarify **who the customers are, what they sell, what role technology plays**, then follow **B)** three columns. Don't lock in business details before confirming company name.

(Consistent with **Q29**; full question jump to **[Q29](#q-29)**.)

<a id="b1-02"></a>
### 2) Team Collaboration & Conflict Resolution
- **Tell me about a conflict with a teammate and how you resolved it.** → **Oral script**: [A ③ Conflict & Review](#oral-03) · [Q6](#q-06) · [Q24](#q-24) · [Q42](#q-42) (**Option B** tech stack/collaboration: **Delphi** vs **team mainstream stack**)
- **Describe a disagreement with your manager.** → **Oral script**: [Q5](#q-05) (**Main: clear disagree + timing/ops risk**) · [Q33](#q-33) (team rollout tension + pilot + production line feedback) · Context [A ⑥ Production vs Milestones](#oral-06)
- **Tell me about a time you had to collaborate cross-functionally.** → **Oral script**: [Q12](#q-12) · [Q20](#q-20) (interface contracts + weekly demos + multi-party alignment)

<a id="b1-03"></a>
### 3) Pressure Management & Prioritization
- **Tell me about a time you worked under pressure.** → **Oral script**: [Q9](#q-09) **Main: Enterprise IM**; **Backup: Air-gapped** skeleton · [A ⑤ Under pressure](#oral-05)
- **How do you prioritize when everything feels urgent?** → **Oral script**: [A ⑥ Prioritize](#oral-06) (including multi-line parallel Case C) · [Q8](#q-08) · [Q36](#q-36)
- **Tell me about a time you had multiple deadlines.** → **Oral script**: [A ⑥ Case C multi-project parallel](#oral-06) · [Q8](#q-08)
- **Urgent work + long-term projects at the same time** → **Oral script**: [Q40](#q-40) (**Smart Factory**: **production interrupts plans** vs **firefighting only = roadmap stalls**; **intentional separation**, **visibility**; **two tracks at different speeds**)
- **Excessive workload / at risk of missing deadline** → **Oral script**: [Q38](#q-38) (**Picture Book**: teammate left → **plan vs reality gap** → **early rebalancing with lead**; **borrower-facing client** vs **cabinet-side/locks/sensors/on-site integration**); estimation error backup **[Q22](#q-22)**

<a id="b1-04"></a>
### 4) Failure & Retrospective
- **Tell me about a time you failed.** → **Oral script**: [A ④ Failure](#oral-04) · [Q4](#q-04)
- **Tell me about a time you missed a deadline.** → **Oral script**: [Q22](#q-22) (same herbal labeling story)
- **Describe a production issue you handled.** → **Oral script**: [A ⑤ Case A production release window triage](#oral-05) · [Creative · Scale](#oral-creative) (device/web link) · Security angle [A ④ Case C avatar incident](#oral-04)

<a id="b1-05"></a>
### 5) Complex Problem Solving & Technical Judgment
- **Tell me about a challenging project and how you handled it.** → **Oral script**: [Q3](#q-03) (**Main: ChatClothes**; **Backup: Smart Factory scale/integration** as *another example* / hardware instability follow-up)
- **Tell me about a complex technical problem you solved.** → **Oral script**: [A ② Complex technical](#oral-02) (including Android OEM extension) · [oral-enterprise-files](#oral-enterprise-files) (FastDFS) · [Q31](#q-31)
- **Tell me about a difficult decision you made.** → **Oral script**: [B1-05 Supplement · Difficult decision](#oral-b1-05-decision) · Can combine with [A ⑨ Case B Architecture Evolution](#oral-09) (Easemob / FastDFS etc.) · [oral-enterprise-files](#oral-enterprise-files)
- **Describe a trade-off you had to make (speed vs quality, etc.).** → **Oral script**: [B1-05 Supplement · Trade-off](#oral-b1-05-tradeoff) · [Q34](#q-34) (Smart Factory: **cloud spec boost + read-write split** for time vs cost; also: **production vs milestones**) · [A ② Extended OEM matrix vs releases](#oral-02) · [oral-enterprise-files](#oral-enterprise-files) (self-hosted files vs public cloud)

<a id="b1-06"></a>
### 6) Initiative & Influence
- **Tell me about a time you took initiative.** → **Oral script**: [Q7](#q-07) **Option B** (**Smart Factory scale** compressed skeleton) · [Creative · Scale](#oral-creative) (expanded); **above and beyond** question → **Q7 Option A Main: ChatClothes**
- **Describe a process improvement you drove.** → **Oral script**: [A ⑦ Process improvement](#oral-07) · [Q44](#q-44) (including CI/CD narrative)
...
<a id="b1-07"></a>
### 7) Feedback & Growth
- **Describe a time you received critical feedback.** → **Oral script**: [A ⑧ Tough feedback](#oral-08) (Case A manager ADR; Case B prototype + decision maker) · [Q13](#q-13)
- **Tell me about a time you gave difficult feedback.** → **Oral script**: [Q21](#q-21)
- **What did you learn from your biggest mistake?** → **Oral script**: [Q4](#q-04) **Main: release config**; **Backup: herbal pilot** (estimation/delay) · [A ④](#oral-04)
- **How do you assure code quality in your team?** → **Oral script**: [Q43](#q-43) (**Main: merged recommended version**; manual-level details **expand on follow-up**)
- **How do you support and track a junior developer's progress?** → **Oral script**: [Q45](#q-45) (**Main: Smart Factory merged recommended version** — **delivery enablement**; **docs/difficult feedback** deep dive → **[Q21](#q-21)** / **[Q13](#q-13)**)

<a id="b1-08"></a>
### 8) Adapting to Change
- **Tell me about a time requirements changed suddenly.** → **Oral script**: [A ⑨ Adapted to change](#oral-09) (Case A ChatClothes; Case B Enterprise IM) · [Q23](#q-23)
- **Describe a major change at work and how you adapted.** → **Oral script**: [A ⑨](#oral-09) · [Q23](#q-23)
- **Tell me about a time you had to learn a new technology quickly.** → **Oral script**: [Q14](#q-14) · [Q25](#q-25) (fast learning outside comfort zone) · **What have you learned recently** see **[Q41](#q-41)** (**practical AI engineering + ChatClothes end-to-end profiling**) · Completely unfamiliar task see **[Q39](#q-39)** (GitHub/docs + **herbal platform** YOLO baseline → platformization)
- **Describe the project workflow in your previous team.** → **Oral script**: [Q44](#q-44) · [A ⑦](#oral-07) · Organizational evolution [A ⑨ Case B](#oral-09) (Lark/approvals etc. mentioned briefly)

<a id="b1-09"></a>
### 9) Customer Orientation
- **Describe a difficult customer/stakeholder situation.** → **Oral script**: [A ⑧ Case B](#oral-08) (prototype + single customer-side decision maker)
- **Tell me about a time you managed unclear requirements.** → **Oral script**: [Q30](#q-30) · [A ⑧ Case B](#oral-08) (on the floor/prototype alignment)
- **How do you handle conflicting stakeholder expectations?** → **Oral script**: [A ⑧ Case B](#oral-08) · [Q30](#q-30)

<a id="oral-b1-05-supplement"></a>
#### B1-05 Supplement (Two questions without standalone Q numbers in Section C)

<a id="oral-b1-05-decision"></a>
**Tell me about a difficult decision you made.**

**Prompt (STAR)**: (T) Self-built messaging infrastructure vs stability; (A) Evaluate **C++** self-built long-term defect cost vs **Easemob** vendor dependency; (R) Core IM migrated to Easemob + **JPush** notification backup + in-house R&D shifted up to platform subsystems.
...
**Prompt (STAR)**: (T) Android full matrix testing vs release cadence; (A) Mainstream gatekeeping + strong signal real devices; (R) Sustainable release rhythm + difficult tickets converge.
...
#### B1-06 Supplement: Influence without authority

**Tell me about a time you influenced without authority.**

**Relationship with [Q5](#q-05)**: Same **Smart Factory** first-version phasing / pilot story; **Q5** addresses **disagreed with manager or a decision** (must signal **disagreement** from the start); **This question** addresses **influencing decisions without formal authority** — can use the short script below, or **adapt** by swapping Q5 main answer's **"disagreed" framing** with **"wasn't the final decision maker, but needed to steer risk"**.

**Prompt (STAR)**: (S) Smart Factory first release — PM wanted more features before launch; (T) No decision authority but needed to drive risk control; (A) Impact table + phasing + pilot data; (R) Team adopted, stabilize first then expand.
...
### B2. STAR Notes (Keywords · mapped to [Oral Section](#section-oral-only) ①–⑩)

> **Numbering note**: This section's **1)–10)** is not the same numbering as **Q1–Q45** in **[Section C](#section-5-scripts)**; "Qxx" always refers to Section C's per-question numbering.  
> **How to use**: Each item below is **keywords to speak**; full English oral scripts are in the corresponding item in **[Section A](#section-oral-only)**. **Section C** long sentences — don't recite directly, adapt for speech first.

<a id="b2-01"></a>
### 1) Tell me about yourself.

**Core profile**: Messy real-world systems → reliable delivery
...
**Closing**: practical AI + engineering delivery
...
**Core tension**: Android background keep-alive vs system killing processes; offline + lightweight vs AI performance

**Key action**: Layered approach (Easemob primary channel + JPush backup) + heartbeat tuning + state sync; local LLM + LoRA optimization; mainstream device gatekeeping + feedback-driven real devices

**Professional trait**: reliability comes from layers, not one trick
...
**Core tension**: Review resistance feeling personal

**Key action**: 1:1 private chat + specific feedback + short pairing

**Professional trait**: fix communication style, not just code
...
**Core tension**: test config shipped to prod

**Key action**: Own the mistake + **env separation** + **multi-person** checks + **progressive automation** + **small updates** follow same verification (systems and checklists, not memory)

**Professional trait**: systemic fixes over blame
...
**Core tension**: **Main**: Enterprise IM **post-launch** bug flood (solo Android); **Backup**: air-gapped **access / process** friction

**Key action**: **reduce uncertainty early** + **structured daily rhythm** + **impact-first triage** + **continuous small releases** + **product/support** **clear expectations** / offline: **prep + checkpoints + honest timelines**

**Professional trait**: **calm operational engineer** — interviewer should hear **organized under pressure**, not **story quantity**
...
**Core tension**: everything looks urgent / ambiguous requirements / multi-project overlap

**Key action**: impact × **operational risk** × **dependency** / **early comms** (expectations) / **practical triage** (deadlines, blockers, what can slip safely) / **escalate or coordinate** before you become the **bottleneck**

**Professional trait**: operationally grounded — **stable systems** + **sustainable** delivery pace
...
**Core tension**: manual releases don't scale with multi-site rollout

**Key action**: Jenkins + GitLab CI/CD + pilot first + team-wide migration

**Professional trait**: less dependent on individual memory
...
**Core tension**: big decisions lived in my head / stakeholder chaos

**Key action**: lightweight ADR + onboarding notes / prototype + single decision owner

**Professional trait**: documentation = long-term feature / noise isn't a requirements doc
...
**Core tension**: constraints changed (offline / hardware) / operational cost too high

**Key action**: measure → re-evaluate design / stop rebuilding commodity infra

**Professional trait**: good systems are rarely static
...
**Core profile**: I stabilize messy real-world systems

**Trigger**：10y production / Android + backend + AI / Enterprise IM + Smart Factory + ChatClothes / operational reality / full-time rights

**Closing**: I enjoy working on difficult real-world problems and helping turn them into stable products people can actually use
...
> Consistent with **[Part 1 · Rules](#section-doc-about)**: **Short sentences, common words, mentally go through STAR**; each question aims for **one-line prompt + one version English main answer**; **Q29–Q45** have **follow-ups + short answer key points** attached. Facts only from KB.

### On-the-Spot (Minimal)
- Didn't hear clearly → **clarify**. One question **~60–120s**: **Action** one more sentence. Don't know → explain **how to investigate, what's the first step**.

### Common Follow-ups (how to use the "follow-up" blocks in this section)
- Interviewers often follow up after main answers with **details / numbers / your role / what if you could do it again / team disagreements**; below **Q29–Q45** have **"follow-up" + "short answer key points"** at the end, on the spot just remember **key points** without memorizing long paragraphs.
- Common English phrasing: **What was the hardest part?** / **How did you measure success?** / **What would you do differently?** / **What was your specific role?** / **Tell me more about the trade-off.**

### How to Read This Section
- **Quick overview table** below; **Q1–Q45** ≠ **B2** numbering; questions overlapping with **A** — **practice one side well**.
- **Follow-ups**: **Q29–Q45** listed at end of each question; **Q1–Q28** can reuse the follow-up phrasing from the start of this section + supplement with your most familiar main story.

### Q1–Q45 Quick Overview (Main Story Tags)

| Q | Topic | Main Story / Keywords |
|---|------|-----------------|
| 1–2 | Strengths / Weaknesses | Product-engineering bridge + ChatClothes; over-explaining → layered explanation + ADR |
| 3 | Challenging project | **Main: ChatClothes** (complexity + constraints + ownership); **Backup: Smart Factory scale/integration** (follow-up *another example* / hardware instability / reliability) |
| 4, 22, 38 | Failure / Delay / Can't finish | **Q4 Main: release test config to prod** (accountability + process + prevention); **Q38** Picture Book — **teammate left + early rebalancing + context-based division** (see body); **Q4/Q22 Backup: herbal labeling** → **pilot** (estimation/delay) |
| 5, 27, 33 | Disagreement | **Q5**: **disagree with manager/decision** — **Smart Factory** (**timing + operational risk** vs adding features to first release); **Q27** colleague; **Q33** team **rollout** strategy disagreement + **pilot/production feedback** correction (same source story, different framing) |
| 6, 24, 42 | Difficult collaboration / Conflict | **Q42**: **Option A** Enterprise IM review freeze; **Option B** **Delphi vs team stack** (weighbridge/entry scale linkage, **oral narratives pending KB ingestion**) |
| 7 | Above and beyond | **Main: ChatClothes** (**not formally required for grading** deployment/API/user docs + **reproducible** + supervisor recognition + **~six months early**); **Backup: Smart Factory scale** → **initiative / creative / process** (see Q7 Option B) |
| 8, 34, 36, 40 | Prioritization / Multiple solutions | **Q8**: **impact + operational risk + dependency**; **communication/expectations**; **multi-line triage**; **escalate early** to prevent **silent bottleneck**. **Q40**: **production vs roadmap** — **both firefighting and long-term must move**, **visibility**, **system stays live during iteration** (see body). **Q34** Smart Factory multi-solution trade-off. **Q36**: sorting + overload — **experience sentence first**, **anti-silent-heroing**, **small team flexibility** (see body) |
| 9 | Pressure | **Main: Enterprise IM** (reduce uncertainty + **structured rhythm** + **continuous builds** + **clear expectations**); **Backup: banknote air-gapped** compressed skeleton (**Q9 Option B**) |
| 10–11 | Work style / Motivation | **Q11**: **real operational problems** + **long-term reliability** + **on-site perceptible** + **ChatClothes = AI under constraints**; **long-lived > demo** |
| 12, 20 | Team / Leading coordination | Interface contracts + integration demos; downplay pure management |
| 13, 21 | Receiving / Giving feedback | **Q13** ADR/onboarding (receiving feedback); **Q21** giving **Junior difficult feedback**: **acknowledge technical work first** → **engineering consequences (handoff cost/review/risk)** → **template + pairing example** → **specific, actionable, non-personal** |
| 14, 25, 41 | Fast learning / Out of comfort zone / New learning | **Q41**: **practical AI engineering** (**AUT + ChatClothes**) — **end-to-end profiling**, **bottlenecks often not inference**; **LLM tools in daily dev**; **demo vs production** converges to **personal exposure**. **Q14**: **ChatClothes** — **paper→run→trace→LoRA** (**~two months** target, **~six weeks** iterable); **Q25** outside comfort zone |
| 15–16 | Agile / Code review | **Q15** Sprint + **demo**; **Q16**: **review = collaborative problem solving**, **ask why first**, **trade-off discussion**, **focus on code not person**, **give specific feedback** |
| 17, 37 | Proud achievement | **Smart Factory** main answer + **Enterprise IM NDK** backup (see Q17/Q37) |
| 18–19, 39 | Risk decisions / Don't know / Brand new task | **Q18**: Friday prod — **slow down first**, **scope/verify/rollback/monitor/release notes**, escalate or **next window** if not confident; truly urgent → **watch logs + rollback ready**. **Q19**: **don't fake knowledge** → **clarify** → **boundaries + investigation path** → **transparent** → **follow up if important**. **Q39** completely unfamiliar task |
| 23 | Major change | **Main: `enterprise-messaging` evolution** (platformization + FastDFS + Easemob + org process + role expansion; **one main thread**); **Backup: ChatClothes** constraint change (skeleton) |
| 26 | DEI product design | **Q26**: **don't pretend to be D&I expert**; **engineering-testable** — **a11y / language / poor network/device** + **early testing & iteration** + **real diverse user feedback**; inclusion = **reliable software usable by broader populations** |
| 28, 35 | Continuous learning | **Q28**: **read + small experiments/POC** + **trace behavior** + **lightweight notes**; **ChatClothes** example (diffusion/LoRA/local deployment). **Q35**: off-hours following + **production-side real usage** filtering + **Android platform drift**; **Q41** **practical AI / profiling / deployment reality** (see body) |
| 29–32 | Why company / Job change | **Q29** Air NZ + [business/JD](#oral-b1-01-why-company-research); Hotter backup needs company name confirmation; **Q32** **main reason: family** (**China→NZ**, new life) → **left previous company**; **MCIS 2026-02** + **stable full-time** + **skill continuity** ([oral-b1-01-why-move](#oral-b1-01-why-move)) |
| 30 | Ambiguous requirements / System design | **Q30**: **workflow + constraints** before **architecture buzzwords**; **observe on the factory floor**; **break into verifiable small pieces**; **prototype**; **make assumptions explicit** (**ADR / spike**); **phased rollout**; **operational validation** |
| 31 | Biggest technical challenge | ChatClothes latency profiling |
| 43 | Code quality | **Main: Q43 merged draft** — **reduce avoidable instability before release**; **Smart Factory** **real device integration demo**; **Enterprise IM** **config accident → env/validation/automation**; **tools ≠ quality**. **Deep dive** unpack: GitLab+Jenkins, Easemob, static scan/MR — see Q43 "expand on follow-up" & **Q44** |
| 44 | Team workflow | **Option A** Smart Factory: **biweekly Sprint** + **interface contracts** + **weekly demos** + **GitLab/Jenkins** pilot rollout ([A ⑦](#oral-07)). **Option B** **`enterprise-messaging`**: platformization evolution rhythm + **Lark/docs/approvals/budget** + **progressive automation**; early **weeks-long prototype→post-launch firefighting** & **product/design transparent to users** (see `facts.yaml` / [A ⑨ Case B](#oral-09)) |
| 45 | Mentor Junior | **Q45 Main: merged draft** — **delivery enablement** (small slices, early integration, visible progress); **check-in**: **what tried / what's stuck / don't take keyboard**; **review** discusses **maintenance cost & integration risk** (**beyond compiler happy**); **light sharing** looks at **trade-off**. **Deep dive**: standups/kanban/30·60·90 → **answer on follow-up**. **Docs line** → **Q21+Q13** |

(This section contains **Q45** scripts total.)

<a id="q-01"></a>
### 1) What is your greatest strength?
**Prompt (STAR)**: Product-engineering bridge + years of hands-on experience + evidence-driven (ChatClothes profiling evidence).


<a id="q-02"></a>
### 2) What is your greatest weakness?
**Prompt (STAR)**: Don't give fake weaknesses; use **as scale grew, context stayed in my head → team handoff & maintenance got harder**, **manager directly pointed it out → agreed → lightweight decision notes + onboarding**; close with **maintainability, handoff, future development** (same source as [A ⑧ Case A](#oral-08)).
...
>
> After that, I changed how I worked. I started writing lightweight decision notes for larger technical choices — not huge documents, just enough to explain the reasoning and trade-offs.
>
> I also added short onboarding notes for modules new developers found confusing.
>
> I still keep docs practical, but now if a decision affects maintainability or onboarding, I make sure the key context is written down.

**On-the-spot (delivery)**: Content is already complete enough; in oral exam don't need **every sentence perfect** — can **shorten sentences**, **pause**, occasionally only answer **two or three sentences**, more conversational than scripted.

<a id="q-03"></a>
### 3) Tell me about a challenging project and how you handled it.
**Prompt (STAR)**: **主答 ChatClothes** — **张力**：初版架构**纸面可行**，**真机 profile** 后 **延迟 + 硬件边界**（含 **LLM / vision**）变成硬约束；**动作**：**不硬扛原设计扩 scale**，按**实测瓶颈**改架构（如 **本地 Ollama**、**vision 路径减负**）；**ownership**：课题**自驱**、**固定**学位截止、**范围**同时盖住 AI / 集成 / 部署；**结果**：系统交付、**约提前六个月**交论文、**First Class Honours**；**反思**：**先测再改**、少猜。主答里**不必早抛**具体小模型全名（技术细节追问见 **[Q31](#q-31)** / `projects/chatclothes/facts.yaml`）。**备用 Option B** — **Smart Factory 秤/串口链路**：更像 **可靠性 + 集成 + 主动性**；适合 **「再来一例 / 不稳定硬件 / 产线侧集成」** 类 follow-up，**不抢**主答的「大挑战」叙事。

**Option A — ChatClothes（主答）**
**Script (spoken)**
> One challenging project I worked on was my master's thesis project, ChatClothes.
>
> The goal was to build a lightweight virtual try‑on system that could run **offline** on constrained hardware like Raspberry Pi‑class devices. The challenge was that the architecture looked fine on paper, but once I profiled the real pipeline, **latency and hardware limits** became serious problems — especially around the LLM and vision steps.
>
> Instead of forcing the original design, I reworked the architecture around the real bottlenecks. I moved the LLM pipeline to a local Ollama‑based setup and simplified parts of the vision workflow so the system stayed practical on smaller hardware.
>
> The project was largely self‑driven with fixed academic deadlines and a large scope across AI, backend integration, and deployment constraints.
>
> In the end, I completed the system, finished the thesis about six months early, and graduated with First Class Honours.
>
> What I learned is that hard engineering gets manageable once you stop guessing and start measuring.

**On-the-spot (delivery)**: 少 **project → tech list → result** 的简历腔；口播抓住 **「难在哪 → 我怎么重新框问题 → 怎么落地」**；可**停顿**、**删半段**，不必一次念满。
...
**Prompt (STAR)**: **主答 Option B 发版配置** — **张力**：小团队、**验证过度依赖人手与记忆**；**动作**：认错 + **快修二次发版** + **环境隔离 / 多人校验 / 渐进自动化**；**反思**：可靠发布靠 **系统与清单**，不靠「有人会记得」。**临场**：少叠方法论标题，**一段事故 + 一段改进** 即可。**Option A 中草药标注** — **估时 / 延期 / 模糊样本** 类题作 **备答**（stakes 偏小，不作「最大失败」默认主叙事）。**Option C 头像上传安全** — **很强但易追问 breach**；**仅 security / infra / IR 向岗位**可作主答，否则 **默认不讲或极短带过**，口径见下（弱化戏剧化，强调 **upload path = security boundary**）。

**Option B — Release config mistake（主答）**
**Script (spoken)**
> One mistake I made earlier in my career happened during an application release.
>
> I shipped an upgrade where the build was still pointing to **test** configs instead of **production**. Users started seeing incorrect data, and we had to push a corrected version quickly.
>
> At that time we were a small team without a mature release process, and too much validation depended on manual checking and individual memory.
>
> I took responsibility, and afterward we tightened the release process. We separated dev/test/prod configs more clearly, added multi‑person verification, and gradually introduced more automation.
>
> Since then I've been much more disciplined about releases, even for small updates. Reliable delivery needs systems and checks, not memory.

**On-the-spot (delivery)**:  spirit 仍是「**清单与系统，而不是靠记忆**」；口播不必 slogan 化，**最后一句**可再缩短半句。
...
**Option C — Bitcoin ransomware incident / security response（可选 · 慎作主答）**

**Key idea:** 仅 **security / platform / incident** 角色或对方**明确深挖安全**时展开；否则易被拽进 **breach 细节** 而弱化 **你的成长**。口径：**professional**，**讲 actions + lessons**，不讲「机器被黑」类论坛感措辞。

**Script (spoken)**
> We faced a Bitcoin ransomware attack that encrypted our data.
>
> I acted quickly to isolate affected servers, restore from clean backups, and implement stronger security measures.
>
> The experience reinforced the importance of rapid response and robust backups.

**Key Points (for quick recall)**
- **Situation**: Bitcoin ransomware attack encrypted our data.
- **Action**:
  - Quickly isolated affected servers to stop further spread.
  - Restored from clean backups.
  - Implemented stronger security measures.
- **Result**: Recovered without paying ransom; environment secured.
- **Lesson**: Rapid response and reliable backups are the foundation of incident recovery.

<a id="q-05"></a>
### 5) Tell me about a time you disagreed with your manager or a decision.
**Prompt (STAR)**: 题干是 **disagreed with manager or a decision** — 前两句就要让人听到 **disagreement**（**首版范围/节奏**：产品要 **更强首交付**，你从 **工程 + 现场运维** 判断 **rollout risk** 过高）。**张力**是 **timing / operational risk**，不是「反对功能本身」（用 **I wasn't against the features themselves** 类句子 **去 ego**）。**做法**：**trade-offs 可见化** + **分期试点** + **现场反馈**，把讨论从主观立场拉到 **证据与共同结果**。**收尾**：主反思句偏完整时略像备稿 → 见下 **临场** 可改更短、更对话。
...
**On-the-spot (delivery)**: 若最后一段嫌「总结稿」感，**二选一**即可：**保留**上面 **reinforced… shared outcomes**；**或**改短为更对话的一句，例如：**After that rollout, I became even more careful about first-release scope whenever the environment is operational and unforgiving.** 不必两句叠念。

<a id="q-06"></a>
### 6) Tell me about a time you worked with a difficult person.
**Prompt (STAR)**: **主**：**Enterprise Messaging**（与 **[Q42 Option A](#q-42)** 同源）；**1:1** + **私下** review + **结对**。**备选**：**技术栈/可维护性分歧** → **[Q42 Option B](#q-42)**。
**Script (spoken)**
...
**On-the-spot (delivery)**: 最后一段若嫌 **keynote 收尾**感，**二选一**：保留 **reinforced… matters too**；**或**改短为更对话，例如：**After that project, I became even more careful about making systems easier for the next person to use or maintain.**
...
**On-the-spot (delivery)**:  credibility 已够 — **少一句 abstract lesson** 也行（例如 **只念到 escalate 那句就停**）；多留 **半拍** 在 Smart Factory 例与 **多项目** 段之间，听起来更像聊天。与 **[A ⑥ Case C](#oral-06)** 可交叉练，不必两处长段完全重复。

<a id="q-09"></a>
### 9) How do you handle stress and pressure?
**Prompt (STAR)**: 考 **情绪稳定 + 排序 + 沟通 + 可持续执行**，不是 **比惨**。**主答**：`enterprise-messaging` **早期单人 Android** — **上线后** **issue 洪峰**；**做法**：**先减不确定**、**结构化日节律**、**按 impact 稳高危**、**连续发小版本**、**产品/支持**帮用户 **对齐期望**。**忌**：一题里 **三段长篇回忆录**（**banknote**、**ChatClothes** 整段叠念）。**Air-gapped banknote mill** → **追问 / second example** 用下方 **Option B 骨架**（见 [A ⑤ Case C](#oral-05) 语境）。**ChatClothes** 的强项在 **profiling / scope / architecture**（**[Q3](#q-03)**、**[Q31](#q-31)**；**显著变化**备答见 **[Q23](#q-23) Option A**），**本题不必**再用「**提前交**」一笔带过当压力主证据。
...
>
> One example was during the early release of an Enterprise Messaging app where I was the only Android developer. We launched quickly, then a wave of bugs came in.
>
> Instead of trying to solve everything at once, I worked in a structured rhythm. Each day I prioritized by **user impact**, stabilized the highest‑risk problems first, and shipped smaller fixes continuously.
>
> Product and support kept expectations clear with users while engineering focused on recovery.
>
> That taught me that a calm, structured rhythm beats emotional reactions when things get hot.

**On-the-spot (delivery)**:  spirit 仍是 **rhythm + clarity beats panic**，**口播别 slogan 化**；主答 **到 recovery 句可停**，**不必**叠念 **banknote**。**Follow-ups**「**还有别的压力吗**」→ 再 **Option B 压缩骨架**。
...
**Prompt (STAR)**: **systems first** → **operational picture** → **evidence before opinion** → **collaborative** → **process improvement**。
...
**Prompt (STAR)**: **不要**答成 **钱/职级/new challenges 空话/泛泛热爱编码**。**能量来源**：**真实运营问题** + **软件长期可靠**（**long-lived systems**）；**环境**：能 **看到** 系统是否在帮人（**Smart Factory 一线**）；**技术兴奋点**：**难**但要 **接真实约束**（**ChatClothes**：不是为 AI 而 AI，而是 **离线 + 硬件边界下能跑**）。**人设**：**practical systems engineer**，不是 hype。**忌**：只有 **keynote bullet** 无场景 — 下面口稿要有 **一两句可画面化** 的例。
...
**On-the-spot (delivery)**: 最后一句若嫌 **interview-polished**，**二选一**：保留 **…depend on the system every day**；**或**改更口语：**I enjoy work where the software keeps mattering after launch, not just during the demo.** 不必两句叠念。

<a id="q-12"></a>
### 12) Tell me about a time you worked effectively in a team.
**Prompt (STAR)**: **跨职能 6 人** → **coordination cost** → **lightweight 结构** → **clear interfaces + steady rhythm** → **10+ 厂多年交付**。
...
**Prompt (STAR)**: **经理反馈** → **shared context 缺失** → **delivery speed vs onboarding** → **lightweight ADR + onboarding guide** → **ramp 更顺 + 重复问题减少**。
...
**Prompt (STAR)**: **情境**：**ChatClothes** 管线需 **扩散 / OOTDiffusion** 与 **LoRA 微调**（见 `projects/chatclothes/facts.yaml`）。**张力**：领域新、**里程碑紧**（自述约 **~two months** 内要跑出可信 **fine-tune** 闭环）。**做法**：**论文 + 官方栈 → 先跑通推理 → trace 耗时/显存/训练热点** → **小步 LoRA 实验**（约 **~six weeks** 进入可迭代跑法）；**PyTorch / Hugging Face** 为可迁移基础。**收束**：快速学习 = **早跑代码、用测量缩小该读什么**，不是纯堆文献。**临场**：追问再展开 **DressCode / VITON-HD** 或指标；口试忌 **名词连读** 无停顿。
...
**On-the-spot (delivery)**: **OOTDiffusion / LoRA** 可念慢一点；**最后一段**若嫌「方法论收尾」感，**可删**，停在 **six weeks… iterate** 也行。

<a id="q-15"></a>
### 15) Are you comfortable working in an Agile team?
**Prompt (STAR)**: **双周 Sprint 六年** → **反馈循环短** → **working software > status reports** → **用户看到真系统后需求演进** → **demo 验证方向**。
...
**Prompt (STAR)**: **不要**答成「我全盘接受」或 **ego 防御**。**收反馈**：**review 是工程机制** → **先理解 comment 背后的 risk/context** → 仍有分歧则 **讲约束与 trade-off**，**不对人**。**给反馈**：**具体**、**落在代码与工程影响**、**说明为何以后会变成问题**。**心理安全**：目标是 **一起把系统变好**，不是 **谁赢辩论**。**临场**：最后一句若嫌 **management tone**，改用更短口语（见下）。
...
**On-the-spot (delivery)**: **最后一段**若嫌 **polished / 领导力腔**，**二选一**：保留 **healthy code review culture…**；**或**改更对话：**I've found reviews usually go much better when people feel the goal is improving the system, not defending their own code.** 不必两句叠念。

<a id="q-17"></a>
### 17) Tell me something you built that you're genuinely proud of.
**Prompt (STAR)**: **主答 Smart Factory** → **不只是软件 demo** → **真生产环境 10+ 厂** → **硬件集成 + 可测量 impact** → **operators felt workflow 更顺** → **长期学习 + 持续交付**。**备用 Enterprise Messaging NDK**：**Tension-Action-Result 结构**。

**Script (spoken)** — **主：Smart Factory（约 45–55s）**
...
**Script (spoken)** — **备用：Enterprise Messaging NDK（约 20–25s）**
...
**Prompt (STAR)**: 考 **production judgment**，不是 **敢不敢点发布**。**态度**：**先降速** — 周五下午 + **support coverage 变弱**，风险阈值应更高。**核对**：影响面、**测试是否覆盖**、**回滚路径是否清楚**、**监控与 release notes** 是否就绪。**不能自信验证恢复路径** → **升级**或 **挪到下一发布窗**，避免 **无谓周五 prod 风险**。**若确属紧急且集体决定上**：**监控打开**、发版后 **紧盯日志**、**回滚步骤随手可执行**。**收束**：靠 **准备与运行纪律**，不靠 **信心或猜**。**spirit**：原有一句 **No guess clicks** 很准 — 口播若嫌 **slogan**，见 **临场** 换成更对话说法。
...
**On-the-spot (delivery)**: **No guess clicks** 可作 **心里锚点**；口播若嫌 **punchline 感**，**二选一**：保留上面 **guesswork / support coverage** 收尾；**或**加一句更松的：**I don't like production deployments that rely on assumptions, especially late on a Friday.** 与正文末段 **二选一** 即可，不必叠满。

<a id="q-19"></a>
### 19) How would you respond if you don’t know the answer to a question?
**Prompt (STAR)**: 考 **未知下是否可靠**，不是 **全知**。**态度**：**不装懂**；**先澄清**题干与语境（真问题常与第一句字面略有偏差）。**做法**：说明 **已知边界**、**不确定从哪开始**、**怎么查证**（文档 / 代码路径 / **生产日志** / 复现 / 问对口的人）。**原则**：**可靠、有条理** 优于 **听起来很确定**；重要事项 **事后 follow up** 给已核实答案 — **unknown 不可怕，失联才可怕**。**临场**：若嫌 **leadership article** 腔，见下 **二选一** 替换句。
...
**On-the-spot (delivery)**: **“reliable and methodical…”** 一段若嫌 **polished**，**二选一**：保留；**或**改更口语：**I've found it's usually better to be clear about uncertainty than to sound confident and be wrong.** 与 **follow up** 句衔接自然即可，不必叠两句价值判断。

<a id="q-20"></a>
### 20) Describe a time when you led a team. What was the outcome?
**Prompt (STAR)**: **技术 leadership**（非 formal management）→ **跨职能 6 人** → **集成风险** → **API contracts** → **weekly demos** → **stable rhythm** → **10+ 厂长期 rollout**。
...
>
> That helped **surface issues earlier**, reduce integration surprises, and keep everyone aligned on what was actually working end‑to‑end.
>
...
>
> After a while we realized **coordination problems were usually more dangerous than the coding itself** — technical leadership is often about **reducing coordination friction** so the team can deliver consistently.

<a id="q-21"></a>
### 21) Describe a time when you had to give someone difficult feedback. How did you handle it?
**Prompt (STAR)**: **不要**答成 **粗暴指责 / 压权威 / 单纯「你做错了」**。**情境**：**Junior** 实现 **技术尚可**，**文档与交接** 薄 → **评审与维护成本** 上升。**做法**：**先肯定**做得好的部分 → **把影响讲具体**（**onboarding 成本、review 变慢、后续改动风险** — **clear cost for the next person**）→ **不只指出问题**：给 **轻量模板** + **结对走一个真实例子**，让期望 **可落地**。**结果**：文档与评审节奏改善，对方也更敢讲清自己的设计取舍。**人设**：**low-ego technical coaching**。**持续机制**可并 **[Q45](#q-45)** 备选「**文档/ADR**」线、**[Q13](#q-13)**。**临场**：若嫌 **HR 式 polished 收尾**，见下 **二选一**。
...
>
> I didn't want the conversation to feel like criticism of their ability, so I started by acknowledging what they were already doing well.
>
> Then I explained the impact more concretely — unclear documentation raises onboarding cost, slows reviews, and makes future changes riskier for the next person.
>
> Instead of only pointing out the problem, I made improvement easier. I shared a lightweight template and paired on a real example so expectations felt practical.
>
> Over time, documentation improved, reviews got smoother, and they became more confident explaining their own design decisions.
>
> That experience reinforced for me that feedback works best when it's specific, actionable, and focused on helping the other person succeed.

**On-the-spot (delivery)**: **“reinforced for me…”** 若嫌 **interview-perfect**，**二选一**：保留；**或**改更口语：**After that, I became more careful about making feedback practical instead of only critical.** 不必两句价值判断叠念。

<a id="q-22"></a>
### 22) Tell me about a time when you missed a deadline. What happened, and how did you handle it?
**Prompt (STAR)**: **中草药 CV 标注项目** → **真实数据 vs 理想假设** → **数据质量差** → **早沟通** → **重设预期** → **pilot batch 测吞吐** → **更可预测**。

**Script (spoken)**
> One time I missed a deadline was during a **computer vision-related labeling project** involving **Chinese herbal data**.
>
> At the beginning, I estimated about **two weeks**, but once we started processing the **real image set**, the **data quality was much messier** than expected — inconsistent photos, unclear boundaries, and more manual cleanup.
>
> Pretty quickly I realized the **timeline was no longer realistic**.
>
> Instead of staying quiet and hoping we could catch up later, I **raised it early**, explained why the estimate changed, and worked with the team to **reset expectations and the schedule**.
>
> I also changed how we approached the work. Instead of estimating on assumptions, I used **smaller pilot batches** to measure real throughput before committing to larger timelines.
>
> The final delivery took **closer to six weeks than two**, but after that the process became **more predictable and less risky**.
>
> After that project, I stopped trusting early estimates until I'd seen real sample throughput.

<a id="q-23"></a>
### 23) Tell me about a time when you had to deal with a significant change at work. How did you adapt to this change?
**Prompt (STAR)**: 考 **随约束与业务演变而调整**，不是 **十年架构演讲**。**主答**：**`enterprise-messaging` 长期演进** — 从 **独立企业 IM** → **共享身份 / 文件 / 权限 + 多子系统的平台**；技术侧 **单机文件 → FastDFS**、**自建消息底层难稳 → Easemob 平台/SDK**（细节追问见 `facts.yaml`）；组织侧 **工具/文档/审批/自动化**（**informal 沟通单独不够 scale**）；个人 **从单线开发 → 跨系统协调、交付与优先级**。**主线一句**（可先在心里念）：**The system stopped being just a chat app and became shared infrastructure.** **忌**：**JPush / 七牛 / 阿里云对标** 等名词 **一口气全倒** — 留追问。**Option A ChatClothes** — **短答 / AI 岗 / technical adaptation** 用 **backup 骨架**；**深叙事**仍可用 **[A ⑨ Case A](#oral-09)**。
...
>
> When the product started, it was basically a standalone enterprise chat system. Over time, business needs changed and the platform expanded into shared identity, file management, permissions, and multiple subsystems.
>
> That change affected both the architecture and the way the team worked.
>
> On the technical side, some earlier decisions no longer scaled. For example, file storage outgrew a single-server model, so we moved toward a distributed FastDFS-based approach for better scalability and control.
>
> Messaging infrastructure also evolved. We originally maintained more low-level messaging components ourselves, but stability and maintenance costs kept growing. Eventually we moved core messaging flows onto Easemob's platform and SDK ecosystem, while our own effort focused on business logic, permissions, workflows, and shared platform services.
>
> The organization evolved too. As the number of projects and teams grew, we introduced more structured collaboration tools, documentation, approvals, and automation processes because informal communication stopped scaling.
>
> Personally, my role changed a lot during that period. I started mainly focused on development work, but over time I became more involved in cross-system coordination, delivery planning, technical trade-offs, and priorities across projects.
>
> What helped me adapt was staying flexible about the original design and re-evaluating assumptions as scale, operational pressure, and business needs changed.

**On-the-spot (delivery)**: 口播 **抓一条主线**（上段首句或 **chat app → shared infrastructure**）；**别**把 **Easemob/FastDFS/子系统** 再扩成第二篇长文。**AI 岗只要短答** → 可只念 **Option A** 四行骨架。
...
**Prompt (STAR)**: **团队内部**（工程 vs 产品 vs 运营）→ **需求漂移 + 定义不清** → **原型对齐** → **单一决策人** → **预防：ownership + prototype + requirement discipline**。
...
>
> Over time, the **team started feeling friction** because **priorities kept shifting** and **different people had different definitions** of what "done" meant. **Engineering risk** also increased because **requirements were changing faster** than the implementation could stabilize.
>
> What helped was moving the **discussion away from abstract debates** and toward **working prototypes and real workflow demonstrations**. Once people could **interact with actual screens**, feedback became **more concrete** and misunderstandings dropped quickly.
>
...
>
> That helped **reduce scope churn**, **stabilize delivery planning**, and **improve collaboration** between engineering, product, and operations.
>
> To help prevent similar problems later, we became more **disciplined about defining ownership**, **validating workflows earlier with prototypes**, and **separating discussion ideas from confirmed requirements**.
>
...
**Prompt (STAR)**: **ChatClothes** → **从 Android/后端/企业软件** → **AI diffusion/模型优化/部署约束** → **跨领域快速学习** → **理论 + 实验 + 迭代** → **小规模实验比纯理论更快**。
...
>
> Before that, most of my experience was in **Android, backend systems, enterprise software, and industrial platforms**. The AI side — especially **diffusion pipelines, model optimization, and deployment constraints** — required me to **learn a very different stack** in a short time.
>
> What helped was **not trying to learn everything theoretically first**. I **combined reading papers with hands-on experimentation** early: **running inference pipelines**, **tracing model code**, **profiling bottlenecks**, **modifying small components**, and gradually understanding how the system behaved in practice.
>
...
**Prompt (STAR)**: **不背** diversity 术语、**不装**全公司 **D&I 项目 owner**。**立场**：从 **工程 + 产品可用性** 答 — **同理心 + 意识** 落在 **可验证行为**。**做法**：**早期**定 **可测** 目标（**对比度、字号缩放、键盘导航（如适用）、读屏、清晰语言、弱网/弱机表现**）；**尽早**让 **不同背景用户** 接触系统 — 很多问题 **只有真实使用才暴露**。**流程**：开发中 **持续测**，别堆到上线前一刻。**收束**：包容设计 **不是单独「功能」**，而是 **让更多人、更多环境下可靠工作** 的软件的一部分。**临场**：若末句 **polished**，见下 **二选一**。
...
**On-the-spot (delivery)**: 最后一句若嫌 **crafted**，**二选一**：保留 **…wider range of people and environments**；**或**改更口语：**I've found a lot of accessibility or usability issues only become obvious once different kinds of people actually use the product.**（与上一段 **real users** 略有重复时 **二选一** 即可。）

<a id="q-27"></a>
### 27) Tell me about a time you disagreed with a colleague. How did you handle the situation?
**Prompt (STAR)**: **工程分歧**（非 stakeholder 冲突）→ **Smart Factory rollout scope** → **产品想要更多功能** → **工程更关注 rollout 稳定性** → **Tension：delivery ambition vs operational risk** → **分阶段发布** → **make trade-offs visible**。
...
>
> From the **product side**, there was **pressure to include more functionality** before the first deployment.
>
...
**Prompt (STAR)**: **不要**答成 **刷 YouTube / 泛泛追新闻 / 只收藏不练**。**习惯**：**读**（论文、**官方文档**、**靠谱博客**、**开源仓库**）— 尤其搞清 **系统底下怎么工作**，而不只 **API 表面**。**关键**：**读不够** → **小原型 / POC**、跑推理、**改一小块看行为**（与 **ChatClothes**、**[Q14](#q-14)** 一致：**hands-on validation**）。**轻笔记**留住 **trade-off**，方便回看。**人设**：**hands-on systems learner**。**临场**：若末句 **polished**，见下 **二选一**。
...
**On-the-spot (delivery)**: 最后一句若嫌 **crafted**，**二选一**：保留 **theory and experimentation together**；**或**更口语：**I usually understand new technology much faster once I've actually built something small with it.**

<a id="q-29"></a>
### 29) Why are you interested in working at [company name]?
**Prompt (STAR)**: **主练 Air New Zealand** → **运营型数字系统**（travel systems, timing, communication, operational flow）→ **软件质量直接影响用户体验** → **engineering culture + long-term ownership** → **与 Smart Factory / enterprise messaging 经验对齐**。**投前**做 **[业务 + JD 对齐](#oral-b1-01-why-company-research)**。备选公司名 **Hotter** 见同锚点「备选」段（**请确认正式英文公司名**后替换）。其他雇主仍可用模板方括号。

**Script (spoken)** — **Air New Zealand（30 秒版）**
> I'm interested in **Air New Zealand** because it's the kind of environment where **software quality shows up in the real customer experience** and **operational reliability at scale**.
>
> That's close to the work I've already done — **enterprise messaging systems**, **Smart Factory platforms**, and **operational tools** where **stability and responsiveness** mattered in production.
>
> I also like **long-term product environments** with **strong ownership and engineering discipline**, so the role feels like a good fit for the kind of systems work I enjoy.

**Script (spoken)** — **Air New Zealand（60 秒版）**
...
>
> In **travel systems**, users **immediately feel** whether the software is **clear, responsive, and reliable** — especially during **real travel situations** where **timing, communication, and operational flow** matter. The impact of good systems is very visible.
>
> That connects closely with the kind of work I've already done. I've spent years building **enterprise messaging systems**, **Smart Factory platforms**, and **operational tools** where **reliability, responsiveness, and stable delivery** mattered in production, not just in demos.
>
> I'm also looking for a **mature engineering culture** with **strong ownership** and **long-term product thinking**. I enjoy working on **systems that evolve over time**, where **maintainability, operational discipline, and collaboration** matter as much as feature delivery.
>
> My background fits that environment because I'm **comfortable working across multiple layers** — **client applications, backend systems, integrations, deployment workflows, and operational support** — especially when **reliability and coordination** are critical.
>
> It feels like the kind of environment where my background would be useful.
**Script (spoken)** — **备选 Hotter（公司全名请自换）**
...
**Prompt (STAR)**: 题干含 **design a system** — 不要只答成 **需求发现**；要带出 **如何结构化思考、降不确定、验证假设、让架构随现实演进**。**起点**：**真实 workflow + 运行约束**（**Smart Factory**：初期常 **ambiguous**，操作者 **痛点多但说不清「理想流程」**）。**做法**：**现场观察**（延迟、信息缺口、摩擦在哪）→ **拆成更小的可验证块**，**避免一次画完美大图** → **快原型** 让用户对 **具体界面/流程** 反应（**people react more accurately to something concrete**）→ 随反馈 **调优先级、改流程、让架构围着线上有效部分演化**。**模糊时**：**假设写清楚** — **轻量 ADR**、高风险区 **spike**、能 **分期** 就不 **一把梭**。**收束**：**快反馈 + 运营验证** 往往比 **过早死磕规格文档** 更降风险。**临场**：中间可插 **一两句现场口吻**（如 **What we noticed pretty quickly was…**），少纯 **retrospective 总结腔**。

**Script (spoken)**
> When I'm asked to design a system, I start by understanding the real workflow and constraints before jumping into architecture.
>
> In a lot of Smart Factory projects, the biggest early challenge wasn't technology — it was ambiguity. Operators knew the workflow was painful, but couldn't always describe the ideal process in technical terms.
>
> So instead of relying only on requirement documents or long meetings, we spent time on the production floor observing how people actually worked, where delays happened, and where friction showed up.
>
> From there, I break the problem into smaller validated pieces instead of trying to design the perfect system upfront. We built quick prototypes and tested workflows directly with users because people react more accurately to something concrete than to abstract discussions.
>
> As feedback became clearer, we refined the workflows, adjusted priorities, and evolved the architecture around what was actually working in production.
>
> When ambiguity exists, I try to make assumptions visible early — lightweight ADRs for major decisions, small spikes for risky areas, and phased rollouts instead of betting everything on one big design.
>
> In complex real-world systems, fast feedback loops and operational validation reduce risk more effectively than perfecting the spec too early.

**On-the-spot (delivery)**: 在 **「车间观察」** 与 **「拆块原型」** 之间，可插半句现场过渡（例如 **People actually struggled with…** / **The workflow looked simple until we watched a shift**），打破纯 **architect retrospective** 节奏；**末段**若嫌满，可**省略** **I've found…** 一句。
...
**Prompt (STAR)**: **主答 Android OEM 碎片化** → **不是单一 bug** → **真实用户环境的长尾** → **lab setup 永远小于 reality** → **分层保活策略 + 备用通道** → **trade-off: release speed vs test depth vs operational risk**。**Backup**: ChatClothes（离线 AI 延迟优化）、FastDFS（分布式文件存储）。

**Script (spoken)** — **主：Android OEM 碎片化（约 45–60s）**
...
>
> The hardest part wasn't the **messaging logic itself** — it was the **real-world Android ecosystem**. Different OEMs like **Xiaomi, Huawei, and OPPO** all **modified Android behavior**, especially around **background execution, battery management, notifications, and process survival**.
>
> A lot of issues **only appeared in real user environments** and couldn't always be reproduced in **emulators or even on devices with the same Android API level**. **Vendor-specific ROM behavior mattered a lot**.
>
> We also had **practical constraints**. We couldn't **test every device and ROM combination** before every release without **blowing up schedule, testing cost, and hardware budget**.
>
> So we had to **balance release speed, test depth, and operational risk**. We focused QA on **mainstream Android versions and high-usage devices** first, then **investigated deeper on strong production signals** — sometimes **buying target phones**, sometimes **reproducing issues directly with users**.
>
> We also **designed the system defensively**: **layered keep-alive**, **backup push channels**, **reconnect logic**, **session restore**, **message re-sync**, and **stronger state reconciliation** after reconnects.
>
> What made the challenge difficult wasn't one bug — it was the **huge long-tail environment** where **the lab setup was always smaller than reality**.
>
> That experience taught me a lot about **operational engineering trade-offs**: **perfect coverage is impossible at scale**, so the real skill is **reducing risk, prioritizing correctly, and responding quickly** when production signals appear.

**Script (spoken)** — **Backup：ChatClothes（AI 延迟优化，约 20s）**
> Another hard AI challenge was making the **entire pipeline usable offline on lightweight hardware**. I originally assumed **diffusion was the main bottleneck**, but **profiling showed cloud LLM latency dominated** the experience. I **moved the LLM local**, **reduced model weight** where possible, and **optimized the vision path** with a lighter YOLO pipeline. The system became **fast enough for real offline demos**, and I finished the thesis early.

**Script (spoken)** — **Backup：FastDFS（分布式文件存储，约 20s）**
...
**Prompt (STAR)** (aligned with `kb/profile.yaml` and your narrative): **NZ stable footing**: **don't complain about previous employer**, **don't fabricate office politics**, **don't fake passion**, **don't be overly personal**, **don't be defensive** — main thread **life transition + career continuity** (**location changed, engineering direction consistent**). **Core tension**: **relocation interrupts career continuity → rebuild long-term engineering path**. **Fact chain**: **family/long-term life** **China → New Zealand** → **left previous role, rebuild career locally in Auckland**; **AUT MCIS** **2026-02** **First Class**, **ChatClothes** (**applied AI + real deployment constraints**, **stayed close to real engineering problems rather than pure academic research**); now seeking **Auckland full-time engineering**. **Value**: Still value **enterprise messaging / Smart Factory** accumulation, emphasize **reliability, scaling, long-term maintenance with real users**, willing to **stay long-term** continuing **Android / backend / operational platforms / AI engineering**. **PSWV** full-time work rights.

**Script (spoken)** — **推荐版（约 45–60s）**
> I'm looking for a new opportunity mainly because my **life changed direction** over the last few years.
>
> I moved from **China to New Zealand for family and long-term lifestyle reasons**, so I **stepped away** from my previous role and **focused on rebuilding locally** here in Auckland.
>
> During that period, I completed my **Master's at AUT with First Class Honours**. My thesis was very **applied** — **AI systems, offline inference, deployment constraints** — so I stayed close to real engineering problems rather than purely academic research.
>
> Now that I've finished the degree, I'm looking to **move back into full-time engineering**.
>
> The direction hasn't changed that much from what I've already spent years doing — **Android, backend systems, operational platforms**, and more recently **AI-related engineering**.
>
> I still **value my previous experience** a lot, especially the **Smart Factory and enterprise messaging work**, because those projects taught me **reliability, scaling, and long-term system maintenance with real users**.
>
> At this point I'm mainly looking to **settle back into long-term engineering work here in Auckland**.

**On-the-spot (delivery)**: **「commit properly to the right role and team」** 若嫌 **HR 腔**，**二选一**：保留；**或**改更自然：**I'm ready to settle back into long-term engineering work here in Auckland.**（与 **full-time work rights** 二选一叠一句即可，不必两句都满。）

**Script (minimal version · ~20–30s）**
...
>
> Since then, I completed my **Master's at AUT** focused on **applied AI and practical systems work**, and now I'm moving back into **full-time engineering in Auckland** — continuing the same kind of **Android, backend, and operational systems** work I've already spent years building.
...
**Script (spoken)** — **展开版（约 60–75s）** — **推荐**

> Disagreements are normal on engineering teams. Most of the time it's not about ego — people are optimizing for different risks.
>
> One example was during the Smart Factory project. We disagreed on rollout strategy. Some people wanted broader functionality in the first release, while I pushed for a smaller but very stable initial launch.
>
> My concern wasn't really the engineering difficulty. It was operational trust. These systems were used directly on factory floors, and once users lose confidence early, it's hard to recover.
>
> So instead of debating endlessly, we narrowed the discussion to practical questions — what could realistically be supported, what failure tolerance production teams had, and what we could safely roll out without creating operational noise.
>
> We ended up doing a phased pilot with a smaller scope first. That gave us real usage data instead of assumptions. Some feature requests mattered less than expected, while a few workflow problems we underestimated became more important once operators used the system day to day.
>
...
**Script (spoken)** — **展开版（约 65–80s）**
> In the **Smart Factory** system, we hit a **scaling problem in production**.
>
> One of the **task tables was growing very quickly** — roughly **over a hundred thousand new rows a day** — and a lot of **portal and reporting queries** depended on it. After a few months, some **screens got slow enough** that **operators on the factory floor noticed** it during daily work.
>
...
>
> **Another option** was to **tolerate the slowdown** for a while, which I didn't think was realistic once **production users were already feeling the impact**.
>
> So we chose a **staged approach** instead.
>
> First, we **moved the workload onto higher-capacity cloud-hosted MySQL** and **separated some of the read traffic** so the operational screens were responsive again. That **increased hosting cost**, but the **trade-off was explicit** — we were **buying stability and time**.
>
> Then, once the **immediate pressure was lower** and the **current feature work shipped**, we **scheduled the slower structural work** — **slow-query analysis, indexing cleanup, and some schema adjustments**.
>
...
**Script (minimal backup · ~22s）** — **产线 vs 里程碑**
...
**Script (spoken)** — **展开版（约 50–60s）** — **推荐**

> Outside work, I mostly learn by building small things and following what people are actually using in production.
>
> I read release notes, engineering blogs, GitHub discussions, sometimes forums if I'm digging into a specific problem. I don't try to track everything — mostly the areas connected to the kind of systems I work on.
>
> Recently that's been a lot of **AI-related tooling** — **local models**, **offline inference**, lightweight agents for coding workflows. Partly from my thesis work, but also because these tools are starting to become **practical for day-to-day engineering work**.
>
> I still keep up with Android because the platform changes constantly, especially around device behavior, permissions, and background restrictions. If you stop paying attention for a while, things drift pretty quickly.
>
> Usually my learning loop is simple. I read something, build a very small prototype around it, and keep short notes if I think it'll be useful later. That's normally enough for me to understand whether something is genuinely practical or just interesting on paper.
...
**提示（STAR）**（已对齐你的表述）：**内核**不变：**影响**、**依赖链**、**不确定性提前**；**不把风险闷在自己手里**（**早暴露**）；团队内 **临时拉人 / 拆活 / 调顺序**，必要时 **砍范围或改日期**。**多线并行** 仍见 [A ⑥ Case C](#oral-06)；与 **[Q8](#q-08)** 可互指。**Oral exam anti-patterns**: 开头不要连打 **工整三元组**（像 **sprint planning 条文**）；**borrow capacity** 易 **PMO 腔** → 口语用 **pull people in / get help from teammates**。**全栈一句** 勿写成 **resume breadth** 广告。**密度**：避免 **每分钟太多 mature insight**（句句像 slogan）；留 **plain sentence**、**未润色措辞**、**边想边说感**。

**Script (spoken)** — **展开版（约 55–70s）** — **推荐**
...
**Script (spoken)** — **主：Smart Factory（约 70–85s）**
...
>
> Over time, the program helped **improve operational efficiency by around 30%** based on the metrics the project tracked, and we **kept the system running reliably** across difficult factory conditions.
>
...
>
> What makes me proud is the **combination of longevity and real usage**. **A lot of software looks good in a demo. This was software people relied on for years.**

**Script (minimal backup · ~35–42s）**
...
>
> For a while I was trying to keep everything moving — legacy issues, new features, hardware-side fixes, ongoing support — but it became clear the original timeline no longer matched reality.
>
...
>
> In smaller teams I've learned it's better to redistribute work early than quietly overload one person and discover the problem near release.
...
**Script (spoken)** — **备选：中草药标注（与 Q4/Q22 同源 · ~20s）**
...
**提示（STAR）**（已对齐你的表述，例：**Smart Factory**）：**张力**：**产线问题会打断计划**；但若 **天天只救火**，**路线图停滞**，**同类运维痛会以别的形式反复出现**。**做法**：**产线正在受影响** 的优先（**当班操作员**、**现场系统** — 仍可自然带一句 **real machines / downtime / money** 类 **physical consequence**，**勿**堆成口号）；**同时** **刻意留出** 给 **长线** 的时间，**进度可碎步**。**认知**：性能、集成清理、流程类 **多在系统在线前提下多轮迭代**，**非一次通关**。**可见性**：别让 **紧急活把路线图完全盖住**，也别让 **路线图无视产线现实**。**延展**：[A ⑥](#oral-06) 产线 vs 路线图。**Oral exam anti-patterns**: 少念 **工整三桶**（urgent / non-urgent / long-term **像 workshop**）；避免 **protect dates / neither track silently dies**、**ops stays calm, milestones still move** 等 **博客腔总结**；**60 秒内不必强行「完整闭环」** — 可 **半收不收**：**yeah, we were mostly trying not to let the roadmap completely die while production was noisy.**
...
**Prompt (STAR)**: **二选一**。**Option A**（同 **Q6/Q24**）：**Enterprise Messaging** — 同事在 **review** 上退缩、参与度低 → **1:1** + **私下具体反馈** + **短结对**。**Option B**（**口述经历**，尚未写入 `projects/*/facts.yaml`；口试 **勿编造** 客户/项目正式名称，可统称 **入口称重/地磅联动** 类交付）：团队里 **资深同事** 习惯用 **Delphi** 做**新项目**（车辆 **上秤称重 → 超重预警 → 联动抬杆/报警** 等现场逻辑）；你当时 **~5–6 年**经验，对方 **行业经验更长**但 **工具链偏旧**、更倾向 **老栈**；你认为 **.NET 等团队已在用的栈** 更利于 **封装复用、协作与工期压缩时他人能接手**。**分歧处理**：先 **1:1** 表达对其 **现场经验** 的尊重，再把讨论落到 **可维护性 / 知识传承 / 工期与接手面** 等**可验证风险**；**同时**承认 **新技术需要学习窗口**。**结果**：**工程/甲方侧决策**采纳 **Delphi 路线** — 你 **书面保留技术顾虑** 后仍 **专业配合交付**，并尽量补 **文档/边界说明** 降低后续维护成本。**Reflection**：**尊重人 ≠ 回避栈决策**；**负责人拍板**后 **执行到位**；长期仍应推动 **团队默认技术栈** 与 **学习节奏** 对齐。

**Script (spoken)** — **Option A（约 25–35s）**
...
**Script (spoken)** — **Option B（约 65–80s）**
...
**Script (spoken)** — **推荐版（融合 A+B · 约 55–70s）**

> For me, code quality is mostly about reducing avoidable instability before software reaches production.
>
> Part of that is technical process — code reviews, CI checks, release validation, environment separation — but a lot of it is engineering discipline over time.
>
...
**Prompt (STAR)**: **二选一**。**Option A** **`smart-factory`**（与 `projects/smart-factory/facts.yaml`、[A ⑦](#oral-07) 一致）：**~六年** **双周 Sprint** — plan、standup、review、retro；每 Sprint **可运行 demo**（**mobile / backend / IoT** 对齐）；**接口契约** 先于破坏性改动；发布从**手动复制**演进到 **Jenkins + GitLab CI/CD**，**工厂项目先试点**再**各交付线统一迁移**，辅以清单与短文档；**团队侧**可安排 **定期技术分享**（新工具/适用问题/优劣与边界），与 Sprint 节奏并行、不必等「项目结束才补课」。**Option B** **`enterprise-messaging`**（与 `projects/enterprise-messaging/facts.yaml`、[A ⑨ Case B](#oral-09) 一致）：产品从**独立 IM** 渐进 **平台化**（用户中心、文件中心、子系统接入）；工程上 **Spring Cloud 微服务 + Node** 多子系统并行；团队扩大与**多项目穿插**后，协作流引入 **飞书（Lark）**、**协作文档**（如用过的 **石墨**）、**PM / 预算 / 审批** 纪律，并 **渐进自动化** 减轻行政摩擦；**极早期**你是 **唯一 Android**，**数周级**原型窗口上线后靠 **每日缺陷清单 + 尽量每日发版** 与 **产品/设计对用户解释预期** 收敛 — 与 **[Q43](#q-43)** 的「**质量 / 发版纪律 / 配置教训**」叙事可同场串讲，勿重复堆砌细节。

**Script (spoken)** — **Option A（约 40–50s）**
...
>
> Another thing we learned over time was to **align API contracts and integration boundaries early** before large changes. Otherwise teams would move fast individually but **collide later during integration**.
>
...
**Script (spoken)** — **Option B（约 50–60s）**
...
Use this section in the last few minutes before an interview. It is a memory trigger, not another full script.
...
- *Reliability usually comes from layers, not one trick.*
...
- 【2026-05-05 新增】`smart-factory` / 真实需求甄别失败与复盘（可用于：失败复盘 / 产品协作 / 用户导向 / 冲突）
  ...
  - Suggested line (EN): "Workers ask for what’s easiest at their station, but we optimize the end-to-end flow. A request is input, not an automatic must-build."
...
- 【2026-05-09 新增】`smart-factory` / 下车间 + 无对标 + 模糊需求下的优先级（可用于：Prioritization / Ambiguity / Stakeholder / 产品迭代）
  ...
  - Suggested line (EN): "We had no reference product, so we earned priorities on the shop floor: fast prototypes, tight feedback loops, and honest replans."
...
- 【2026-05-10 新增】`banknote-paper-mill` / 造币印纸厂断网现场与联调压力（可用于：Under pressure / Ambiguity / Stakeholder / 交付节奏）
  ...
  - Suggested line (EN): "One plant visit cost a week of paperwork. You couldn’t patch like the cloud — offline bundles, controlled transfer, and rollback had to be designed in."
- 【2026-05-12 新增】`cross-project` / 多项目并行：外部交付 vs 自营产品线（可用于：Prioritization / Time management / Stakeholder / 资源协调）
  ...
  - Suggested line (EN): "External deadlines and internal product lines don’t share one inbox — I pick today’s fire by who’s waiting and what’s still unknown, and I borrow hands early."
- 【2026-05-13 新增】`enterprise-messaging` / Android 厂商 ROM 与版本碎片化、复现与测试策略（可用于：Complex technical / Trade-off / Quality / 客户问题）
  ...
  - Suggested line (EN): "Users outnumbered our test rack — we balanced ship date, test hours, and how many phones we could own; mainstream gates first, then the exact device path when it mattered." 
- 【2026-05-15 新增】`enterprise-messaging` / 自建文件平台：FastDFS、容量与私密性、多子系统文档（可用于：Complex technical / Architecture / Trade-off / Security 与运维交叉）
  ...
  - Suggested line (EN): "Attachments and contract PDFs don’t belong on one disk forever — we scaled with self-hosted FastDFS, and we still owned redundancy, backups, and access control."
...
- 【2026-06-08 从 Gemini 对话提取】`smart-factory` / 第三方 OTA 模块 bug → 新设备无法升级 → 自写升级方案（可用于：Under pressure / Problem-solving / 技术判断）
  - Situation: Smart Factory 项目使用第三方 OTA 升级模块；某次发布后发现新设备批次无法完成升级。
  - Task: 用户均已在使用中，无法升级意味着功能与安全补丁无法送达；必须尽快定位。
  - Action: 在用户真机上联调定位，确认第三方模块兼容 bug；自行编写升级方法替换并重新发包。
  - Result: 新设备恢复可升级路径；此后每次发布前优先验证升级功能正常再测其他修复。
  - Suggested line (EN): "The third-party OTA module failed on newer devices. I debugged on a user's real device, wrote our own upgrade path. Now upgrade check is always my first pre-release test."

- 【2026-06-08 从 Gemini 对话提取】`smart-factory` / 数据库慢查询 → 临时上云 → 本地修复后迁回（可用于：Under pressure / Trade-off / Problem-solving / 成本意识）
  - Situation: Smart Factory 原型上线后某核心表超百万记录，部分查询超 30 秒，客户无法接受。
  - Task: 在不能停线、不能等的前提下让系统可用，并控制长期成本。
  - Action: 短期将数据库临时迁移至云端托管并提升配置规格，缓解慢查询；严重 bug 收敛后定位根因（索引、分表策略），修复后迁回本地。
  - Result: 用户侧性能立即改善，团队获喘息窗口；长期通过新旧记录分表避免持续云费用。
  - Suggested line (EN): "We bought time with cloud -- upgraded DB so users weren't waiting 30 seconds per query, then fixed indexing and partitioning properly, brought it back on-prem when the math made sense."

- 【2026-06-08 从 Gemini 对话提取】`cross-project` / NZ vs China 工作文化差异精炼版（可用于：Why NZ / Cultural fit / Relocation）
  - Core contrast: In China, work culture often involves longer hours and high pace to meet aggressive deadlines. In New Zealand, strong emphasis on work-life balance -- complete responsibilities during standard hours, prioritize personal time and well-being.
  - Extended: China tech market is highly competitive and moves at incredibly fast pace; NZ offers greater focus on work-life balance, flat org structure, and collaborative teamwork.

- 【2026-06-08 从 Gemini 对话提取】`personal` / Weakness: 没有长期职业路线图，一直被任务推着走（可用于：Weakness / Self-awareness）
  - Honest angle: I haven't always had a long-term career roadmap. In China I was pushed by endless projects. I didn't take time to define my direction. Moving to NZ gave me space to reflect. Now I'm actively focusing -- choosing depth in one direction rather than spreading across every new technology.
  - Suggested line (EN): "For years I let projects push me instead of me steering my own direction. Now I'm learning to pick depth over breadth."

- 【2026-06-08 从 Gemini 对话提取】`cross-project` / "Why should we hire you?" 融合精炼版（可用于：Elevator pitch / Why hire）
  - Script: I bring over ten years of production experience across backend, mobile, and IoT. I've delivered systems where reliability matters: smart factories where the shop floor can't wait, enterprise messaging at real user scale. I recently specialized in applied AI at AUT. Based in Auckland with full work rights, looking for a team where I can contribute strong execution, reliability, and practical problem-solving.
  - 口试时可按目标公司裁剪首尾两句；中间事实段保持不变。
