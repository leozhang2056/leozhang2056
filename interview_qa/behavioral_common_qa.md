...
> **你补充新故事时**：先写在下方 [**个人素材补全区**](#section-personal-drafts)，核对 `projects/<id>/facts.yaml` 与 `kb/profile.yaml` 后再把精炼版合并进口语 / STAR / 第五节，并更新 [故事库映射](#section-story-map)。
...
| [**个人素材补全区**](#section-personal-drafts) | 你新增的 STAR 草稿、追问预案、与 KB 的对应关系 |
...
| 现场交付 / 硬件集成 / 网络波动 | **Smart Factory**（RFID/电子秤/串口、多现场部署） | **Forest Patrol**（离线地图、弱网下巡检流程） |
| 模糊需求 / 快速澄清 / 先做可运行版本 | **Smart Factory**（先核心流程，后增量） | **Chinese Herbal Recognition**（先闭环工作流，再扩展域） |

### 项目事实速查卡（面试前 5 分钟）

> **目的**：把「讲得顺」和「说得准」合在一起；每个项目都准备 1 条主线 + 2 个追问点。  
> **口径来源**：`projects/*/facts.yaml` 与 `kb/achievements.yaml`，以下均可直接口语化改写。

#### 1) chatclothes（研究到可运行系统）
- **一句主线**：独立完成多模态虚拟试衣系统，从模型改造到部署闭环，6 个月完成并提前 6 个月提交。
- **可说动作**：LoRA 微调 OOTDiffusion、YOLO12n 改为 YOLO12n-LC、本地 DeepSeek + Ollama、在 Raspberry Pi 5 运行端到端流程。
- **可核验结果**：`FID 28.5（较基线提升 19%）`、`分类准确率 94.2%`、`总延迟 <10s（Raspberry Pi 5）`、`IVCNZ 2025 论文 DOI`。
- **常见追问答点**：为什么先优化 LLM 路径而不是扩散模型；如何在「速度/质量/可离线」间做取舍。

#### 2) smart-factory（长期生产交付）
- **一句主线**：6 年持续迭代的智能工厂平台，覆盖 Android + 后端微服务 + 工业 IoT，支撑多工厂落地。
- **可说动作**：电子秤串口监听、watchdog 与自动重连、ActiveMQ/Kafka 事件协同、Docker + Jenkins 多站点发布。
- **可核验结果**：`30%+ 效率提升`、`关键流程高可用（常用口径 99.9%）`、`6 人跨职能团队协作`、`多站点 rollout`。
- **常见追问答点**：如何做上线前范围控制；当生产问题与里程碑冲突时怎么排序。

#### 3) enterprise-messaging（低延迟与长期稳定）
- **一句主线**：企业 IM 平台 10 年生命周期，Android/Web/PC 多端协同，重点攻克低延迟与稳定性。
- **可说动作**：NDK C/C++ 实现 TCP/UDP 通道、JNI 桥接、连接池与心跳机制、跨端发布协同。
- **可核验结果**：`5,000 DAU`、`500K+ 消息/天`、`<200ms 消息时延`、`10 年持续可用`。
- **常见追问答点**：为何要下沉到 NDK；高并发下如何做故障隔离与回退。

#### 4) chinese-herbal-recognition（失败与复盘高频素材）
- **一句主线**：搭建中药识别平台闭环（标注→训练→评估→发布→推理），并在标注估算失误后建立可复用防错机制。
- **可说动作**：把标注、训练、评估、模型版本与回滚串成一条流程；引入小样本 pilot 校准工时与质量。
- **可核验结果**：形成可扩展到其他视觉分类场景的平台式流程；版本生命周期与发布门禁标准化。
- **常见追问答点**：如何定义标注质量门槛；出现延期时如何对齐预期并保住质量底线。
...
> **用途**：你往这里贴**新故事、新指标、新复盘**；整理满意后，再同步到 [口语专页](#section-oral-only)、[第二节 STAR](#section-2-star-brief)、[第五节脚本](#section-5-scripts)，并视情况改 [故事库映射](#section-story-map) 一行。  
