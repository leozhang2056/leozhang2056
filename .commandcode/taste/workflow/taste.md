...
- Full CV generation pipeline should be automated end-to-end: save JD → generate CV → generate CL → clean up intermediates → git commit → git push — all without waiting for user prompts between steps. User explicitly stated: "Every time user provides a JD, generate both CV and CL automatically without asking. Save JD first, then generate CV + CL in sequence." Confidence: 0.95
- For GitHub README.md, only use GitHub-supported Mermaid syntax — avoid experimental chart types (e.g., `xychart-beta`); use `<br/>` → `\n` for line breaks inside node labels; and never use a node ID that collides with its subgraph name (e.g., don't name a node `IoT` inside `subgraph IoT`). User reported rendering errors across multiple rounds and asked for fixes. Confidence: 0.90
